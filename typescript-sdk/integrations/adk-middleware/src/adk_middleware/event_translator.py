...
class EventTranslator:
    ...
    def __init__(self):
        ...
        self._accumulated_streaming_content: str = ""  # Track content accumulated during streaming
    ...
    async def _translate_text_content(
        ...
        if is_final_response:
            ...
            # Check if this final content matches what was already streamed
            content_already_streamed = (self._is_streaming and
                                        self._accumulated_streaming_content and
                                        self._accumulated_streaming_content == combined_text)
            
            # Send complete message in 3 events if:
            # 1. We weren't streaming (no previous content), OR  
            # 2. The final content is different from what was streamed
            should_send_complete_message = (not self._is_streaming or 
                                            not content_already_streamed)

            if should_send_complete_message and should_send_end:
                logger.info(f"⏭️ Deliver complete message via 3 events "
                            f"event_id={adk_event.id}, was_streaming={self._is_streaming}, "
                            f"content_matches={content_already_streamed}")
        ...
        if not self._is_streaming:
            # Start of new message - emit START event and reset accumulated content
            self._streaming_message_id = str(uuid.uuid4())
            self._is_streaming = True
            self._accumulated_streaming_content = ""
        ...
        # Always emit content (unless empty) and accumulate it
        if combined_text:
            # Accumulate the content for comparison with final response
            self._accumulated_streaming_content += combined_text
    ...
    def reset_streaming(self):
        """Reset the translator's streaming state.
        """
